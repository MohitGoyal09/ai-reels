...
from transformers import pipeline # !pip install transformers sentencepiece
...
# --- 2. Define VibeClassifierHFZeroShot (same class definition as before) ---
class VibeClassifierHFZeroShot:
    def __init__(self, vibe_taxonomy_json_path: str, 
                 model_name: str): # Model name passed directly
        ...
        try:
            ...
            # Define more descriptive candidates for better zero-shot classification
            ...
            # Create enhanced candidate labels for better classification
    ...
    def classify_vibes(self, text_to_classify: str, confidence_threshold: float = 0.50, multi_label: bool = True):
        ...
        try:
            # Create a better hypothesis template for fashion vibes
            ...
            # Map back from enhanced candidates to original vibe names
            labels_to_original = {}
            for i, label in enumerate(classification_results['labels']):
                # Extract the original vibe name (before the colon)
                original_vibe = label.split(':', 1)[0] if ':' in label else label
                labels_to_original[label] = original_vibe
            
            # Create scored labels with original vibe names
            scored_labels = [
            ...
            # Use a scoring mechanism that favors clear winners
            # If one vibe is significantly higher than the rest, only return that one
            if len(sorted_scored_labels) > 1:
                top_score = sorted_scored_labels[0]['score']
                second_score = sorted_scored_labels[1]['score']
                
                # If top score is 15% higher than second, only return the top vibe
                if top_score > second_score * 1.15:
                    detected_vibes_final = [sorted_scored_labels[0]['label']]
                else:
                    # Otherwise return up to 3 vibes
...
# Only run the test code if this file is executed directly, not when imported
if __name__ == "__main__":
    ...
    # --- Ensure dummy vibe list exists if real one not uploaded ---
    ...
    # --- Initialize VibeClassifierHFZeroShot with facebook/bart-large-mnli ---
    ZS_MODEL_NAME = "facebook/bart-large-mnli" # Switched to BART-large
    ...
    # --- List of Video IDs to Test (Same as before) ---
    ...
    if vibe_classifier_hf_instance:
        ...
        # --- STARTING CONFIDENCE THRESHOLD FOR BART-LARGE-MNLI (NEEDS TUNING) ---
        ZERO_SHOT_CONFIDENCE_THRESHOLD = 0.50 # BART-large might have different score distributions
    
        # Helper function to get combined text (copied from previous test harness)
        def _get_combined_text_for_video_id_test(video_id: str, captions_dir: Path, metadata_dir: Path) -> str:
            raw_caption_from_txt = "" # ... (full implementation from previous response) ...
        ...
        for video_id in video_ids_to_process:
            ...
            # Create dummy files for this ID if they don't exist for the test to run
            # It's always better to use your actual files.
